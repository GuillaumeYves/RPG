module ApplicationHelper
  def display_item_tooltip(item)
    content_tag(:div, class: "tooltip-content #{item.rarity&.downcase}", id: "tooltip_#{item.id}", style: 'min-width: 400px;') do
      ...
      concat(content_tag(:div, "Level : #{item.level_requirement}", class: 'item-level_requirement')) if item.level_requirement.present?
    ...
    attributes.each do |attribute|
      ...
      if item.public_send(attribute).present?
        ...
        if attribute == 'critical_strike_chance'
          ...
          base_value = "#{(base_value * 100).round(2)}%"
        elsif attribute == 'global_damage'
          base_value = number_to_percentage(base_value * 100, precision: 2)
        end
        ...
        if upgraded_value.present? && upgraded_value != 0
          if base_value
            ...
            upgraded_html = "(+<span'>#{upgraded_value_display}</span>)"
            html += content_tag(:div, "#{attribute.humanize}: #{base_value} #{upgraded_html}".html_safe, class: 'tooltip-item-attribute')
          end
        else
          html += content_tag(:div, "#{attribute.humanize}: #{base_value}", class: 'tooltip-item-attribute')
        end
      end
    end
